...
**Implementation constraints (carry forward verbatim to the Claude Code implementation session, where they're enforced by `CLAUDE.md` and by code-review tools on the resulting PR):**

- **MCMC chains are sequential — do not vectorize the outer sampling loop.** Per-step linear algebra (Cholesky factorization of $G(\theta)$, metric-vector solves, leapfrog half-steps) is fine to express as NumPy vector/matrix operations, but the loop over the $N$ samples is intrinsically serial — step $t+1$ depends on the state at step $t$. The implementation session will reject any proposal that tries to vectorize the per-sample iteration.
- **Hoist constants and buffer allocations out of the inner sampling loop.** Allocate scratch arrays (momentum vector, intermediate leapfrog position, Cholesky factor of $G(\theta)$ when it's reused across a half-step, working memory for the implicit fixed-point solver) *once* before the loop and reuse with in-place writes. This matches the project-wide rule for Newton / ISTA / power-iteration loops in `src/components/viz/shared/` — see `CLAUDE.md` § "Hoist `Float64Array` allocations out of Newton / ISTA / power-iteration loops."
- **Exploit Fisher symmetry when materializing $G(\theta)$ at high $d$.** $G$ is symmetric by construction (covariance of scores), so when the metric is computed elementwise rather than from a closed-form block expression, compute only the lower triangle and mirror to the upper. For the banana ($2 \times 2$) and multivariate-Gaussian (block-diagonal) demos the materialization is trivial and the trick is moot; for the log-Gaussian Cox-process application $G$ can sit at the dimension of the discretized intensity field, where the half-triangle saving starts to compound.
...
The brief should also include an **"Eventual MDX frontmatter" subsection** near the top, specifying the canonical YAML the downstream Claude Code session will write into `src/content/topics/riemann-manifold-hmc.mdx`. The current canonical field set (match the most-recently-shipped topic — `density-ratio-estimation.mdx` as of PR #93) is: `title`, `subtitle`, `abstract`, `domain`, `difficulty`, `prerequisites`, `tags`, `connections`, `references`, `videoId`, `notebookPath`, `githubUrl`, `datePublished`, `estimatedReadTime`, plus the cross-site arrays `formalcalculusPrereqs`, `formalstatisticsPrereqs`, `formalcalculusConnections`, `formalstatisticsConnections`. Do **not** propose the deprecated `description` field, and do not propose older brief-draft fields the codebase has retired (`slug`, `track`, `order`, `keywords`). The brief documents the frontmatter; it does not write MDX itself — that's the Claude Code session's job.
...
5. **Viz design intent** — as many interactive visualizations as the §-by-§ outline naturally produces; **don't artificially cap**. Recent T5 shipments are 8–12 per topic (VBMS shipped 10, sparse-bayesian-priors shipped 11, SG-MCMC shipped 10). For each viz: what it shows, what parameters the reader can manipulate, what the reader should learn. Component-level *intent*, not React/D3 implementation. Roughly one viz per H2 that has interactive content; some sections (computational notes, connections-and-limits) are text-only and intentionally lack viz. For any viz where a slider would drive an expensive MCMC simulation (step size, leapfrog-step count, target dimension, number of samples), specify the **display-vs-committed** state pattern in the viz brief: the slider's label and any cheap live-preview panel update on `onChange` for instant feedback, but the heavy `useMemo` recompute only fires on `onMouseUp` / `onTouchEnd` / `onKeyUp`. Reference implementations on the live site: `SGLDBatchSizeExplorer.tsx`, `FiniteSampleBiasExplorer.tsx`. The pattern is documented project-wide in `CLAUDE.md` § "Slider perf: commit-on-release for heavy MC."

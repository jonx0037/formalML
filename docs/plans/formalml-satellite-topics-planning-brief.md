...
**Convention for forthcoming topics.** When a topic references another not-yet-published formalml topic, use plain text with an italic annotation per `CLAUDE.md`: `**Kernel Methods** *(coming soon)*`. (The original draft of this brief proposed an `[...](empty)` link pattern; that was superseded during the v1.1 strategic doc revision in favor of the plain-text convention used by the rest of formalML's tooling.) Inline references in formalstatistics MDX prose use a different convention — backticked slug with a parenthetical, e.g., `` formalml `kernel-regression` (forthcoming) `` — but that is a sister-site convention; formalML-side prose follows CLAUDE.md.

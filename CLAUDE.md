...
**`/paths` is fully data-driven.** When publishing a topic: (1) set `status: "published"` in the MDX frontmatter, (2) remove the slug from its track's `planned` array in `src/data/curriculum.ts`, (3) flip the curriculum-graph node's `status` to `"published"`. The `/paths` page picks up everything else automatically — track card flips from "N planned" to "K published / N-K planned", a `<TopicCard>` renders, and the slug disappears from the bullet list. No `paths.astro` edits needed. Steps (2) and (3) are easy to forget (different files from the MDX you just wrote) and `pnpm audit:cross-site` does NOT verify status consistency — only the live `/paths` rendering does. Drift manifests as the topic appearing both as a `<TopicCard>` *and* as a stray "Planned" pill (PR #93 → PR #94 fix). Always preview `/paths` before merging a topic-shipment PR.

...
3. **The odd-vs-even degree pattern (§5).** Combining the first two: there is no uniform-rate benefit to fitting an even-degree polynomial over the next-lower odd degree once boundary behavior is included. Local-quadratic gives the same uniform $O(h^3)$ rate as the next-higher odd $p = 3$'s interior parity-bonus, but the design-adaptive structure that odd $p$ enjoys is gone. The practical choices are $p = 1$ and $p = 3$, and the choice between them comes down to whether you need derivative estimates.

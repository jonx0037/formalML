...
connections:
  ...
  - topic: "statistical-depth"
    relationship: "Tukey halfspace depth's axiomatic theorems and the convexity of depth contours (statistical-depth Proposition 1) live entirely in the language of supporting hyperplanes, halfspaces, and convex hulls. The convex-analysis toolkit is the natural setting for derivations of the Tukey median's geometric properties."
references:
  - type: "book"
...
| **[Quantile Regression](/topics/quantile-regression)** | The pinball loss $\rho_\tau(u)$ is piecewise-linear convex, and its empirical risk minimization is the canonical reduction of a piecewise-linear convex objective to a linear program — via the standard slack-variable split $u = u^+ - u^-$ with $u^+, u^- \geq 0$ (the canonical convex-analysis device). |

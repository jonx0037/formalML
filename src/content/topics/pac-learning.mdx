...
connections:
  ...
  - topic: "conformal-prediction"
    relationship: "PAC bounds give *learning* guarantees (low expected loss) under iid assumptions; conformal prediction gives *coverage* guarantees under the strictly-weaker exchangeability assumption. The two operate on orthogonal axes — predictability versus per-prediction reliability — and the choice depends on whether the downstream task cares about average error or per-prediction validity."
references:
  - type: "paper"
...
The key insight is that an infinite hypothesis class, when restricted to a finite sample, can only produce finitely many distinct labeling patterns. The effective complexity of $\mathcal{H}$ on $n$ points is not $|\mathcal{H}|$ but the number of distinct behaviors on those points. This leads us to the VC dimension — introduced here and developed in full at [VC Dimension](/topics/vc-dimension), including the Sauer–Shelah lemma, the Fundamental Theorem of Statistical Learning, and worked examples for canonical classes.

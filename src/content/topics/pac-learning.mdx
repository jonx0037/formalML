...
**Inductive step.** Assume the lemma holds for all pairs $(m', d')$ with $m' + d' < m + d$ and $m' > d'$. Let $C = \{x_1, \ldots, x_m\}$ be any set of $m$ points achieving the maximum $\Pi_{\mathcal{H}}(m) = |\mathcal{H}_C|$. Set $C' = C \setminus \{x_m\} = \{x_1, \ldots, x_{m-1}\}$.
...
| **Bayesian Nonparametrics** *(coming soon)* | Probability & Statistics | Bayesian model selection provides an alternative to SRM for balancing model complexity; posterior contraction rates parallel PAC sample complexity |

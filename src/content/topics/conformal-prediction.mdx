...
<TheoremBlock type="remark" title="Covariate Shift Connection — Tibshirani-Barber-Candès-Ramdas 2019">
...
{/* PLACEHOLDER: ExchangeabilityBreakdown viz anchor — Task 2.5 will replace
     this comment with <ExchangeabilityBreakdown client:visible /> plus a
     <noscript> static fallback. The viz visualizes split conformal's coverage
     loss under covariate shift and the importance-weighted variant's recovery,
     directly illustrating Remark 3 (Covariate Shift Connection). */}

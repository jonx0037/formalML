...
**VC dimension** ($\dim_{\mathrm{VC}}(\mathcal{H})$). For binary classification, defined as the size of the largest set $\mathcal{H}$ can shatter — fully developed in [VC Dimension](/topics/vc-dimension) and introduced in [PAC Learning](/topics/pac-learning). VC dimension is a *distribution-free, worst-case* measure: it captures the largest possible generalization gap over all data-generating distributions. The Vapnik-style SRM bound (§4) uses VC dimension — universal but typically loose.

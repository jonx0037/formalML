...
Let $p(x, \theta)$ be a joint density on $\mathcal{X} \times \Theta$, $x \in \mathcal{X}$ a fixed observation, and $q$ a density on $\Theta$ absolutely continuous with respect to $p(\cdot, \theta)$. The *evidence lower bound* of $q$ at $x$ is
...
The within-block factorizations across $k$ and $i$ are not additional assumptions — they follow from the model's conditional independencies given the other block.

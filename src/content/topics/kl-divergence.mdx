...
- **Rate-Distortion Theory** *(coming soon)* — the minimum rate $R(D)$ for encoding a source at distortion level $D$ is an optimization over mutual information — itself a KL divergence — subject to a distortion constraint. The Blahut–Arimoto algorithm iteratively minimizes KL divergence to compute $R(D)$.

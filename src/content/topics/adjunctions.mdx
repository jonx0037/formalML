...
**Example: Floor ⊣ Inclusion.** Let $P = \mathbb{R}$ (or a subset) with the usual order, $Q = \mathbb{Z}$ with the usual order, $f = \lfloor \cdot \rfloor$ (floor), and $g = $ inclusion $\mathbb{Z} \hookrightarrow \mathbb{R}$. Then:

$$\lfloor x \rfloor \leq n \quad\Longleftrightarrow\quad x \leq n \qquad \text{for all } x \in \mathbb{R}, \, n \in \mathbb{Z}$$

Check: $\lfloor 2.7 \rfloor = 2 \leq 3$ and $2.7 \leq 3$. Both hold. $\lfloor 2.7 \rfloor = 2 \leq 2$ and $2.7 \leq 2$. The first holds but the second doesn't? Wait — $2.7 \leq 2$ is false, and $\lfloor 2.7 \rfloor = 2 \leq 2$ is true. The equivalence seems to break! But look carefully: we need $n$ to be an integer, and indeed $\lfloor 2.7 \rfloor \leq 2$ is true while $2.7 \leq 2$ is false. The issue is that the equivalence only holds when $q$ is in the *right* poset. Here, the right poset is $\mathbb{Z}$, and the right adjoint $g$ is the inclusion $\mathbb{Z} \hookrightarrow \mathbb{R}$, so the condition is $x \leq g(n) = n$. Since $n$ is an integer and $2.7 > 2$, we have $x \not\leq g(n)$, consistent with... well, actually $\lfloor 2.7 \rfloor = 2 \leq 2$ *is* true. So we need to recheck: is the floor function really left adjoint to inclusion?

The correct statement is: $\lfloor x \rfloor \leq n \iff x < n + 1 \iff x \leq n$ when $n$ is an integer (since $x \leq n$ for integer $n$ is equivalent to $\lfloor x \rfloor \leq n$). Verify: $\lfloor 2.7 \rfloor = 2 \leq 2$ is true, and $2.7 \leq 2$ is false. So $2 \leq 2$ but $2.7 > 2$ — the equivalence fails? No! We've been careless. The correct adjunction is:

$$\lceil x \rceil \leq n \iff x \leq n \qquad \text{(ceiling ⊣ inclusion: wrong direction)}$$

Actually, the standard Galois connection is $f(p) \leq q \iff p \leq g(q)$ where $f$ is a left adjoint. For floor and inclusion, we have $\lfloor x \rfloor \leq n \iff x \leq n$ only when $x$ is not an integer with $\lfloor x \rfloor = n$... Let's be precise. The correct statement is simply: $\lfloor x \rfloor \leq n \iff x < n + 1$, and since $n$ is an integer, $x < n+1 \iff x \leq n$ when $x$ is not an integer. For integer $x$: $\lfloor x \rfloor = x \leq n \iff x \leq n$. So the equivalence does hold: $\lfloor x \rfloor \leq n \iff x \leq n$ for all $x \in \mathbb{R}$ and $n \in \mathbb{Z}$.
...
**Galois connection verification: floor ⊣ inclusion.**
...
for x, n in test_pairs:
    lhs = (np.floor(x) <= n)   # floor(x) <= n
    rhs = (x <= n)             # x <= n
    print(f"  floor({x}) = {int(np.floor(x))} <= {n}: {lhs}  "
          f"  {x} <= {n}: {rhs}  "
          f"  Match: {lhs == rhs}")
```
...
| **Monads & Comonads** *(coming soon)* | Every adjunction $F \dashv G$ generates a monad $T = GF$ with unit $\eta$ and multiplication $\mu = G\varepsilon F$. The Eilenberg-Moore and Kleisli categories provide canonical adjunctions that recover a monad — establishing the fundamental correspondence between adjunctions and monads. |

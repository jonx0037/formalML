...
The visual story is in Figure 4. The two log-likelihood surfaces over $(\log\tau, \log\sigma)$ have different optima — REML's optimum sits at slightly larger variance components than ML's, exactly the bias correction (4.4) was designed to produce. On the six-classroom data with $p = 2$, the correction is small (~3% on $\sigma^2$, slightly larger on $\tau^2$ because of the $\log\det \mathbf{X}^\top \mathbf{V}^{-1}\mathbf{X}$ term's nonlinear interaction with $\tau^2$). On a study with $p = 10$ and $N = 30$, the same comparison produces ML estimates 30% lower than REML's, which is the regime where REML is non-negotiable.
...
### 5.1 The Bayesian GLMM specification
...
**Claim 1: variance-component posteriors track REML.** The non-centered NUTS fit produces 4000 posterior draws of $(\tau, \sigma)$. Plot the marginals as densities, overlay vertical reference lines at the REML estimates from §4 and the truth. The posterior mean of $\tau^2$ should be within a few percent of $\hat\tau^2_{\mathrm{REML}}$, and the posterior mean of $\sigma^2$ should be within a fraction of a percent of $\hat\sigma^2_{\mathrm{REML}}$ — the variance-component posteriors are concentrated near the REML estimates, which is what "the data is informative enough" looks like for $J = 6, N = 60$.

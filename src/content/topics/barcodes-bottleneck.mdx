...
domain: "topology"
videoId: null
notebookPath: "notebooks/barcodes-bottleneck/barcodes-bottleneck.ipynb"
githubUrl: null
...
for a, b, c in [('Circle', 'Cluster', 'Figure-Eight')]:
    d_ab = bottleneck(h1_dgms[a], h1_dgms[b]) if len(h1_dgms[a]) > 0 and len(h1_dgms[b]) > 0 else 0
    d_bc = bottleneck(h1_dgms[b], h1_dgms[c]) if len(h1_dgms[b]) > 0 and len(h1_dgms[c]) > 0 else 0
    d_ac = bottleneck(h1_dgms[a], h1_dgms[c]) if len(h1_dgms[a]) > 0 and len(h1_dgms[c]) > 0 else 0
...
for sigma in noise_levels:
    ...
    for _ in range(n_trials):
        ...
        d_b = bottleneck(base_dgm, noisy_dgm) if len(noisy_dgm) > 0 else 0
        d_bs.append(d_b)
        d_hs.append(d_h)

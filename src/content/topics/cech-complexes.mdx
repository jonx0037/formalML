...
In $\mathbb{R}^d$, the Čech complex $\check{C}_\varepsilon(P)$ has dimension at most $d$. A subset $\sigma \subseteq P$ belongs to $\check{C}_\varepsilon(P)$ if and only if every sub-subset of size $\leq d+1$ does. This means the Čech complex in $\mathbb{R}^d$ is determined by its $d$-skeleton.
</TheoremBlock>

...
Recall from [Simplicial Complexes](/topics/simplicial-complexes), Definition 6: the Vietoris-Rips complex at scale $\varepsilon$ includes a subset $\sigma$ as a simplex if all pairwise distances are at most $2\varepsilon$ (using the ball-radius convention). The critical difference:

...
- **Adjunctions** *(coming soon)* — An adjunction $F \dashv G$ is defined by a natural isomorphism $\mathrm{Hom}(FA, B) \cong \mathrm{Hom}(A, GB)$. The unit $\eta: \mathrm{Id} \Rightarrow GF$ and counit $\varepsilon: FG \Rightarrow \mathrm{Id}$ are natural transformations satisfying the triangle identities. The free-forgetful adjunction between **Set** and **Vec** (and its generalizations) is the categorical formalization of "optimal" constructions.

- **Monads & Comonads** *(coming soon)* — A monad $(T, \eta, \mu)$ consists of an endofunctor $T$ and natural transformations $\eta: \mathrm{Id} \Rightarrow T$ (unit) and $\mu: T^2 \Rightarrow T$ (multiplication) satisfying associativity and unit laws. The Giry monad on **Meas** provides the categorical foundation for Bayesian inference.

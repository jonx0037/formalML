...
<Figure
  ...
  alt="Exact null distribution of the Wilcoxon signed-rank statistic for n=8, with the observed W+ = 28 marked and the two-sided rejection region shaded; a Gaussian approximation overlays the discrete bars"
  caption="Exact null distribution of $W^+$ for $n = 8$, enumerated over all $2^8 = 256$ sign vectors. The observed $W^+ = 28$ falls in the upper tail but not at the rejection threshold for $\alpha = 0.05$. The interactive `SignedRankNullBuilder` (below) lets you drag the $D_i$ values to see how the histogram and rejection region update; for $n \leq 15$ the enumeration is fast enough to be live."
/>
...
<Figure
  ...
  caption="Permutation $p$-values from $800$ datasets generated under $H_0$ (both groups iid from the same distribution): the histogram is close to uniform on $[0, 1]$, and the Kolmogorov-Smirnov test for uniformity gives $p_{\mathrm{KS}} \approx 0.45$ — strongly consistent with Theorem 3. Empirical rejection rates at nominal $\alpha \in \{0.01, 0.05, 0.10\}$ are $0.004, 0.028, 0.084$ respectively, all below the nominal level. The interactive `PermutationDistributionExplorer` (below) lets you swap in different test statistics — mean difference, Welch $t$, Mann-Whitney $U$, max correlation — and watch the permutation distribution and $p$-value redraw."
/>
...
<Figure
  ...
  caption="Empirical null distribution of $U$ from the $\\binom{12}{6} = 924$ rank assignments at $n_1 = n_2 = 6$, with the Gaussian approximation $\\mathcal{N}(18, 39)$ overlaid. The observed $U = 18$ sits at the mode. At $n_1 = n_2 = 20$, $5{,}000$ Monte Carlo permutations give empirical mean $200.76$ vs theoretical $200$, and empirical variance $1363.79$ vs theoretical $1366.67$ — agreement to three significant figures."
/>
...
<Figure
  ...
  caption="Empirical power of Wilcoxon vs $t$-test as a function of shift $\theta$ for three error densities: normal ($\mathrm{ARE} \approx 0.955$, Wilcoxon slightly behind $t$), Laplace ($\mathrm{ARE} = 1.500$, Wilcoxon $50\%$ more efficient), and $t_5$ ($\mathrm{ARE} \approx 1.241$). The curves cross at $\theta = 0$ where both tests have $\alpha = 0.05$ size, then separate. The interactive `AREExplorer` (below) lets you sweep across many density families and shape parameters to feel the full landscape."
/>
...
<Figure
  ...
  caption="The $n = 8$ paired-difference example with one outlier ($D_8 = 8.0$). The $36$ Walsh averages span $[-0.30,\, 8.00]$; their median is $\hat\theta_{\mathrm{HL}} = 1.20$, sitting between the sample median ($1.15$) and the sample mean ($1.85$). The $95\%$ HL confidence interval is $[0.30,\, 4.45]$. The interactive `WalshAveragesExplorer` (below) lets you drag points to feel the contrast between the three location estimators under contamination."
/>
...
<Figure
  ...
  caption="Empirical type-I error and power across four parent distributions: normal, Laplace, $t_3$, contaminated normal. All three tests (Welch $t$, permutation $t$, Mann-Whitney $U$) hold the nominal $\alpha = 0.05$ size in each case (rejection rates between $0.038$ and $0.060$ across $500$ Monte Carlo replications with $80$ permutation samples per replication). Power differences track Pitman ARE: Welch $t$ wins narrowly under normality, Mann-Whitney pulls ahead under $t_3$ and the contaminated normal."
/>
...
<Figure
  ...
  caption="**Left:** under heavy-tailed log-normal revenue ($\sigma_{\log} = 2$), Welch's $t$-test sits noticeably **below** nominal $0.05$ across all sample sizes — skewed metrics distort the Welch-Satterthwaite reference, systematically under-rejecting. The randomization inference (permutation $p$-value) sits at $0.05$ across all $n$. **Right:** CUPED with $\rho_{X, Y} = 0.7$ lifts power substantially. At an effect where raw permutation reaches $50\%$ power, CUPED-adjusted permutation reaches $\sim 80\%$ — equivalent to a $50\%$ reduction in required sample size, achieved purely from the pre-period covariate. Randomization exactness carries through (Remark 6); the CUPED-adjusted test remains exact at level $\alpha$ by Theorem 12."
/>

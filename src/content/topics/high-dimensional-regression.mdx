...
import FistaConvergenceTrace from '../../components/viz/FistaConvergenceTrace.tsx';
...
import RidgeLassoEnetPaths from '../../components/viz/RidgeLassoEnetPaths.tsx';
import DebiasedLassoCoverage from '../../components/viz/DebiasedLassoCoverage.tsx';
...
<FistaConvergenceTrace client:visible />
...
<RidgeLassoEnetPaths client:visible />
...
<DebiasedLassoCoverage client:visible />
...
- [Semiparametric Inference](/topics/semiparametric-inference). The DML framework introduced in §12.1 generalizes substantially: any target parameter $\theta_0$ identified by a moment condition with high-dim nuisance can be estimated $\sqrt n$-consistently using cross-fitted ML estimates of the nuisance. The lasso is one of several admissible nuisance estimators (alongside random forests, gradient boosting, neural networks). The semiparametric-inference topic treats the unified framework — Neyman orthogonal scores, the $o_p(n^{-1/4})$ nuisance rate condition, the practical pipeline — at the level needed for applied econometrics and causal inference; §7.3 of that topic cites the Bickel–Ritov–Tsybakov $\sqrt{s \log p / n}$ rate from this topic as a black-box input.

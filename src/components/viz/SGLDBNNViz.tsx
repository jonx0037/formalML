...
// SGLDBNNViz.tsx
//
// §6 Stochastic-Gradient Langevin Dynamics. Four panels — three spatial + one
// diagnostic — on a 60×60 Two Moons grid:
//   (a) SGLD predictive mean (RdBu_r heatmap) over post-burn-in samples.
//   (b) SGLD predictive standard deviation (viridis heatmap).
//   (c) Sampled 0.5-probability decision boundaries (paletteBNN.sgld).
//   (d) Single-component weight trace + autocorrelation function — the
//       practical mixing diagnostic that distinguishes SGLD from a fitted
//       posterior approximation. The brief calls this out as a §6 deliverable
//       (Q4: trace-plot + autocorrelation panels).
//
// Interactive controls:
//   - log₁₀ η slider (-7..-5, default ≈ -6.3 → η ≈ 5e-7) — step size. Smaller
//     η → less noise, slower mixing; larger η → more noise, faster but
//     coarser exploration. Re-runs the chain on commit.
//   - Samples slider S (50..400, default 200) — post-burn-in chain length.
//     Longer chains give smoother predictive mean / std.
//   - Resample button — re-rolls the chain seed.
//
// Implementation: in-browser via sgMCMCBNNTraining (full-batch SGLD). The MAP
// is used as the warm-start so we skip the initial chase-from-init phase that
// would otherwise inflate burn-in.
// =============================================================================
...
const FIGURE_PATH = '/images/topics/bayesian-neural-networks/06_sgld_two_moons.png';
const ALT =
...
const ARIA = 'Figure 6: Interactive SGLD predictive on Two Moons';
...
interface CachedChain {
  result: SGMCMCResult;
  gridProbs: Float32Array[]; // S forward passes through the grid
  eta: number;
  S: number;
  chainSeed: number;
}
...
async function runChain(
  ...
  await yieldToBrowser();
  const result = sgMCMCBNNTraining(
    ARCH,
    TRAINING_BASE(chainSeed),
    cachedMAP.data,
    {
      method: 'SGLD',
      eta,
  ...
  // Forward each sample through the grid (S × ~700k flops ≈ 140 MFLOP at S=200)
  ...
  return { result, gridProbs, eta, S, chainSeed };
}
...
type CompState =
  | { kind: 'idle' }
  | { kind: 'training-map' }
  | { kind: 'sampling'; cachedMAP: CachedMAP; reason: 'first' | 'eta' | 'S' | 'reseed' }
...
export default function SGLDBNNViz() {
  ...
  // First-hydration: train MAP, then run initial chain.
  useEffect(() => {
    let alive = true;
    ...
    trainMAP(seedRef.current).then((cachedMAP) => {
      ...
      runChain(cachedMAP, Math.pow(10, DEFAULT_LOG10_ETA), DEFAULT_S, chainSeed).then((cachedChain) => {
  ...
  const launchChain = useCallback(
    (cachedMAP: CachedMAP, eta: number, samples: number, seed: number, reason: 'eta' | 'S' | 'reseed') => {
      const ticket = ++ticketRef.current;
      setState({ kind: 'sampling', cachedMAP, reason });
      runChain(cachedMAP, eta, samples, seed).then((cachedChain) => {
  ...
  const handleLogEtaCommit = (next: number) => {
    ...
    setLogEta(next);
    if (state.kind === 'ready') launchChain(state.cachedMAP, Math.pow(10, next), S, chainSeed, 'eta');
  };
  const handleSCommit = (next: number) => {
    if (next === S) return;
    setS(next);
    if (state.kind === 'ready') launchChain(state.cachedMAP, Math.pow(10, logEta), next, chainSeed, 'S');
  };
  const handleReseed = () => {
    const next = chainSeed + 1;
    setChainSeed(next);
    if (state.kind === 'ready') launchChain(state.cachedMAP, Math.pow(10, logEta), S, next, 'reseed');
  };
        ...
        {!cachedMAP || !cachedChain ? (
          <FallbackWithBanner
            label={
              state.kind === 'training-map'
                ? 'Training MAP for SGLD warm-start (~5 s)…'
                : state.kind === 'sampling' && state.reason === 'first'
                  ? 'Running initial SGLD chain (~3 s)…'
                  : state.kind === 'sampling'
                    ? 'Re-running SGLD chain…'
                    : 'Loading…'
            }
          />
        ) : (
          <>
            <Controls
              ...
              isBlocked={isBlocked}
            />
            <Panels cachedMAP={cachedMAP} cachedChain={cachedChain} S={S} width={width} />
          </>
        )}
      </div>
      <figcaption
    ...
    >
      ...
      <label style={{ display: 'inline-flex', alignItems: 'center', gap: '0.4rem' }}>
        ...
        <input
          ...
          style={{ width: '120px' }}
          disabled={isBlocked}
          aria-label="Number of SGLD samples"
        />
...
function Panels({
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
      ...
      >
        <Panel title="(a) SGLD mean" width={spatialW} height={spatialH}>
          <Heatmap imgURL={meanURL} width={spatialW} height={spatialH} data={cachedMAP.data} />
        </Panel>
        <Panel title="(b) SGLD std" width={spatialW} height={spatialH}>
          <Heatmap imgURL={stdURL} width={spatialW} height={spatialH} data={cachedMAP.data} dataStroke="rgba(255,255,255,0.85)" />
        </Panel>
        <Panel
          title={`(c) ${Math.min(SAMPLED_CONTOURS_PER_PANEL, effectiveS)} sampled boundaries`}
          width={spatialW}
          height={spatialH}
        >
          <Boundaries
            ...
            height={spatialH}
            data={cachedMAP.data}
            stroke={paletteBNN.sgld}
          />
        </Panel>
      </div>
      <Panel title="(d) Trace + autocorrelation (chain mixing diagnostic)" width={diagW} height={diagH}>
        <TraceACF
